<!-- resources/views/admins/vouchers/create.blade.php -->
<!DOCTYPE html>
<html>
<head>
    <title>Tạo Voucher</title>
    <link href="{{ asset('css/app.css') }}" rel="stylesheet"> <!-- Điều chỉnh đường dẫn CSS nếu cần -->
    <style>
        .container {
            max-width: 600px;
            margin: 50px auto;
    ...
    <div class="container">
        ...
        <form action="{{ route('admin.vouchers.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <button type="button" class="btn btn-generate" onclick="generateCode()">Tạo mã</button>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="type" id="type" required>
                    ...
                    <option value="percent" {{ old('type') == 'percent' ? 'selected' : '' }}>Giảm giá theo phần trăm</option>
                </select>
            </div>

            <div class="form-group">
                <label for="sale_price">Giá trị giảm</label>
                <input type="number" step="0.01" name="sale_price" id="sale_price" value="{{ old('sale_price') }}" required>
            </div>

            <div class="form-group">
                <label for="min_order">Giá trị đơn hàng tối thiểu</label>
                <input type="number" step="0.01" name="min_order" id="min_order" value="{{ old('min_order') }}" required>
            </div>

            <div class="form-group">
                <label for="max_price">Giá trị giảm tối đa</label>
                <input type="number" step="0.01" name="max_price" id="max_price" value="{{ old('max_price') }}" required>
            </div>
            ...
            <button type="submit" class="btn">Tạo Voucher</button>
        </form>
    </div>
